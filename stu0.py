...
class iq3_cmd(sleekxmpp.ClientXMPP):
    ...
    def set_cc(self,cc):
        ...
        resp['channel']=cc
        try:
            out = self['iq3'].set_cc(self.jid, self.to, self.Resource, cc)
            try:
                resp['error'] = out.xml.find('{foxtel:iq}epg_managment/{foxtel:iq}error').text
            except:
                resp['error'] = None

            try:
                resp['response'] = out.xml.find('{foxtel:iq}epg_managment/{foxtel:iq}response').text
            except:
                resp['error'] = 'Unknown error'
