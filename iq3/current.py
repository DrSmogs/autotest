...
from iq3 import stanza, current_programme, diagnostic_tuner, current_viewing, system_information, get_volume, set_volume, epg_managment
...
class iq3(BasePlugin):
    ...
    def set_cc(self, jid=None, tjid=None, resource=None, cc=None):
        seqnr = "1234567"
        iq = self.xmpp.Iq()
        iq['from'] = jid + "/" + resource
        iq['to'] = tjid + "/" + resource
        ...
        iq['type'] = 'set'
        iq['xml:lang'] = 'en'
        iq['epg_managment']['video_cc']['test'] = cc
        iq.enable('epg_managment')
        self.sessions[seqnr] = {"from": iq['from'], "to": iq['to'], "seqnr": seqnr, "name": "epg_managment", "namespace": "foxtel:iq"};
        resp = iq.send(block=True)
